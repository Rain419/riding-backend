...
public class UserController {
	...
	public JsonResult uploadCar(@RequestBody Car car) {
		...
		//判断车辆信息不为空
		//车辆图片可以为空
		if(StringUtils.isBlank(car.getDriverId()) || StringUtils.isBlank(car.getCarNumber()) ||
				StringUtils.isBlank(car.getBrand()) || StringUtils.isBlank(car.getModel()) ||
				StringUtils.isBlank(car.getColor()) ) {
	...
	public JsonResult uploadPassengerIcon(@RequestBody Passenger passenger) {
		...
		//判断乘客id是否存在 储存头像
		if (userservice.findUserIdIsExist(passenger.getUserId())) {
